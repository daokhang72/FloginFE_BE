...
// File: frontend/src/tests/validation.test.js
import { validateUsername, validatePassword } from '../utils/validation';

describe('Login Validation Tests', () => {
    test('TC_LOGIN_001: Username rong hoac khoang trang', () => {
        expect(validateUsername('')).toBe('Ten dang nhap khong duoc de trong');
        expect(validateUsername('   ')).toBe('Ten dang nhap khong duoc de trong');
    });

    test('TC_LOGIN_004: Username chua ky tu dac biet', () => {
        expect(validateUsername('user@123')).toBe('Ten dang nhap chi chua chu cai va so');
    });

    test('TC_LOGIN_009: Password thieu chu cai (Chi co so)', () => {
        expect(validatePassword('12345678')).toBe('Mat khau phai chua ca chu cai va so');
    });
});
\end{lstlisting}
...
\begin{lstlisting}[language=Java, breaklines=true, basicstyle=\footnotesize\ttfamily]
// File: backend/src/test/java/.../service/AuthServiceTest.java
...
\begin{lstlisting}[language=Java, breaklines=true, basicstyle=\footnotesize\ttfamily]
// File: frontend/src/tests/productValidation.test.js
import { validateProduct } from '../utils/productValidation';

describe('Product Validation Tests', () => {
    const validProduct = {
        name: 'Laptop Dell', price: 15000000, quantity: 10,
        description: 'Mo ta ngan gon', categoryId: 1
    };

    test('TC_PROD_005: Gia am hoac bang 0', () => {
        expect(validateProduct({ ...validProduct, price: -5000 }).price)
            .toBe('Gia san pham phai lon hon 0');
        expect(validateProduct({ ...validProduct, price: 0 }).price)
            .toBe('Gia san pham phai lon hon 0');
    });

    test('TC_PROD_008: So luong la so thap phan (Float)', () => {
        expect(validateProduct({ ...validProduct, quantity: 10.5 }).quantity)
            .toBe('So luong phai la so nguyen');
    });

    test('TC_PROD_014: San pham hop le hoan toan', () => {
...
\begin{lstlisting}[language=Java, breaklines=true, basicstyle=\footnotesize\ttfamily]
// File: backend/src/test/java/.../service/ProductServiceTest.java
