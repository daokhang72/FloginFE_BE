...
describe('ProductForm Component Tests', () => {
    ...
    test('Hien thi form chinh sua va dien san du lieu', () => {
        ...
        expect(previewImg).toHaveAttribute('src', expect.stringContaining('old-image.jpg'));
    });

    // TC3: Upload ảnh mới (Test handleFileChange)
    test('Upload anh moi va hien thi preview', () => {
        render(
            <ProductForm 
                onSave={mockOnSave} 
                onCancel={mockOnCancel} 
        ...
        // Kiểm tra ảnh preview mới xuất hiện
        const previewImg = screen.getByAltText('Preview');
        expect(previewImg).toBeInTheDocument();
    ...
    test('Submit form thanh cong khi du lieu hop le', () => {
        ...
        // Điền dữ liệu
        fireEvent.change(screen.getByLabelText(/Tên sản phẩm/i), { target: { value: 'New Product' } });
        fireEvent.change(screen.getByLabelText(/Giá/i), { target: { value: '100000' } });
        fireEvent.change(screen.getByLabelText(/Số lượng/i), { target: { value: '10' } });
        fireEvent.change(screen.getByLabelText(/Danh mục/i), { target: { value: '1' } });
    ...
    test('Hien thi loi validation va khong goi onSave', () => {
        ...
        expect(screen.getByText(/Giá không hợp lệ/i)).toBeInTheDocument();
    });
    ...
    test('Xoa loi validation khi nguoi dung nhap lieu', () => {
        ...
        // 2. Nhập liệu lại
        fireEvent.change(screen.getByLabelText(/Tên sản phẩm/i), { target: { value: 'A' } });
