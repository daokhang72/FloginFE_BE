...
function ProductForm({ productToEdit, onSave, onCancel, categories }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      console.log("Phát hiện lỗi validation:", validationErrors);
    ...
    <div className="form-container">
      <form onSubmit={handleSubmit} encType="multipart/form-data">
        ...
        <div className="input-group">
          ...
          {errors.name && <span className="error-message" style={{color: 'red', fontSize: '0.9rem'}}>{errors.name}</span>}
        </div>
        ...
        <div className="form-row">
          ...
          <div className="input-group">
            ...
            />
            {errors.price && <span className="error-message" style={{color: 'red', fontSize: '0.9rem'}}>{errors.price}</span>}
          </div>
          ...
          <div className="input-group">
            ...
            />
            {errors.quantity && <span className="error-message" style={{color: 'red', fontSize: '0.9rem'}}>{errors.quantity}</span>}
          </div>
        </div>
        ...
        <div className="input-group">
          ...
          {errors.categoryId && <span className="error-message" style={{color: 'red', fontSize: '0.9rem'}}>{errors.categoryId}</span>}
        </div>
        ...
        <div className="input-group">
          ...
          ></textarea>
          {errors.description && <span className="error-message" style={{color: 'red', fontSize: '0.9rem'}}>{errors.description}</span>}
        </div>

        {/* --- ẢNH --- */}
        <div className="input-group">
          <label>Hình ảnh</label>
          <input type="file" onChange={handleFileChange} accept="image/*" data-testid="product-image-input" />
          
          {/* Preview Ảnh Đẹp Hơn (như bạn đã sửa) */}
          {previewImage && (
            <div className="image-preview-container">
              <p style={{margin: '0 0 10px 0', fontSize: '0.9rem', color: '#888'}}>Xem trước ảnh:</p>
        ...
        <div className="form-actions">
          <button type="submit" className="btn btn-primary" data-testid="submit-btn">Lưu</button>
